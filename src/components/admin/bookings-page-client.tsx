...
import {
  BookingsViewToggle,
  type BookingsView,
} from "./bookings-view-toggle";
import { WeekCalendar } from "./week-calendar";

const STORAGE_KEY = "admin:bookings:view";
...
export interface AdminBooking {
  ...
  locationType: "store" | "customer" | null;
  address: string;
...
export function BookingsPageClient({ bookings }: Props) {
  ...
  const handleChange = (v: BookingsView) => {
    setView(v);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8 gap-4 flex-wrap">
        ...
        <BookingsViewToggle view={view} onChange={handleChange} />
      </div>
...
function BookingsList({ bookings }: { bookings: AdminBooking[] }) {
